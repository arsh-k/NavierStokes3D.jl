...
"""
    navier_stokes_3D_solver_sphere_xpu(; do_vis, do_save)
...
"""
@views function navier_stokes_3D_solver_sphere_xpu(; do_vis=false, do_save=true)
    # Physics
    ...
    for it = 1:nt
        ...
        if do_vis && it % nvis == 0
            ...
            p3=heatmap(xc,zc,Array(C)';aspect_ratio=1,xlims=(-lx/2,lx/2),ylims=(-ly/2,ly/2),title="C")
            p4=heatmap(xc,zv,Array(Vz)';aspect_ratio=1,xlims=(-lx/2,lx/2),ylims=(-ly/2,ly/2),title="Vz")
...
Sets a spherical object in the three-dimensional domain with a no-slip surface.
"""
...
# Solver Call
navier_stokes_3D_solver_sphere_xpu()