...
"""
@views function navier_stokes_3d_solver_sphere_multixpu_weak_scale(; do_save=false)
    # physics
    ...
    for it = 1:nt
        if (it == 11) t_tic = Base.time(); niter_ws = 0 end
    ...
    t_it  = t_toc / niter_ws                  # Execution time per iteration [s]
    @printf("Time = %1.3f sec, (niter_ws = %d)\n", t_toc, niter_ws)
...
navier_stokes_3d_solver_sphere_multixpu_weak_scale()
